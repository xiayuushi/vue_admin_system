...
const actions = {
  async actionsfnLogin ({ commit }, v) {
    ...
    commit('mutationsfnSetToken', res.data.data) // 将请求回来的数据替换修改原来state中的初始数据
  },

  async actionsfnGetUserInfo ({ commit }) {
    const res = await sysProfile()
    commit('mutationsfnSetUserInfo', res) // 获取用户信息用于验证token正确性
    const res2 = await sysUserId(res.data.data.userId) // 获取头像
